...
$(".turnchallenge").mouseover(function () {                     
        if (turn == 0){                                         //if rotation is inactive then indicate the button on mouseover
 $(".turnchallenge").html("Press for secret challenge")}
});
$(".turnchallenge").mouseleave(function() {                     //if rotation is inactive then revert to "difficulty" on mouseleave
    if (turn == 0){
 ...
 $(".turnchallenge").html("Press to stop rotation");            //changes header text to Press stop rotation after it's been clicked.
    if (turn ==  0) {                                           //if rotation is inactive then start rotation.
     var angle = 0;
     $(".circle").addClass("rotate");
turner = setInterval(function() {                               //function found at: http://jsfiddle.net/YgBMa/1
    $(".rotate")
...
turn = 1} else {                                                //determine rotation is active | if active then set to inactive.
angle = 0;
turn = 0;
